/**
 * BONUS138 GUIDE PWA v1.0.0 - Google Apps Script Sync
...
function doGet(e) {
  ...
  if (action === 'ping') return jsonResponse({ ok: true, message: 'BONUS138 GUIDE GAS Sync aktif', time: new Date().toISOString() });
...
function doPost(e) {
  try {
    ...
    if (body.action === 'saveConfig') {
      ...
      logAction('saveConfig', 'ok', 'Config updated from PWA v1.0.0');
...
function getOrCreateMediaFolder_(tag) {
  const rootName = 'BONUS138_GUIDE_MEDIA';
