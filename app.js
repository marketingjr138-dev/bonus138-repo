...
function attachImageFallback(img){
  ...
  img.addEventListener("error", () => {
    ...
    if(img.id === "brandLogo"){
      img.src = "assets/logo-bonus138.png";
    }
...
function bindEvents(){
  ...
  $("exportConfig")?.addEventListener("click",()=>{const clean={...config}; delete clean.__localOverride; const blob=new Blob([JSON.stringify(clean,null,2)],{type:"application/json"}); const a=document.createElement("a"); a.href=URL.createObjectURL(blob); a.download="bonus138-guide-config-v1.0.0.json"; a.click(); URL.revokeObjectURL(a.href);});
